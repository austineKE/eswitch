...
public interface TransactionRepo extends CrudRepository<TransactionRequests, Integer> {
    @Query(nativeQuery = true, value = "SELECT * FROM transaction_requests where  transaction_completed=0 and processed=1 and send_money_successful=0 and send_money_retry_count<2  ORDER BY id asc limit 100;")
